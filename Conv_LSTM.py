...
class CLSTM_cell(nn.Module):
    ...
    def __init__(self, shape, input_chans, filter_size, num_features):
        ...
        self.Gates = nn.Conv2d(self.input_chans + self.num_features, 4 * self.num_features, self.filter_size, 1,
                              self.padding)
    ...
    def forward(self, input, hidden_state):
        ...
        # print ("combined_size:{}".format(combined.size()))
        gates = self.Gates(combined)
        # print("gates:{}".format(gates.size()))
        # (ai, af, ao, ag) = torch.split(A, self.num_features, dim=1)
        ai, af, ao, ag = gates.chunk(4, 1)# it should return 4 tensors
        # print("ai:{}".format(ai.size()))
        # print("af:{}".format(af.size()))
        # print("ao:{}".format(ao.size()))
        # print("ag:{}".format(ag.size()))
        ...
        # print("next_h:{}".format(next_h.size()))
        # print("next_c:{}".format(next_c.size()))
        return next_h, next_c

    def init_hidden(self, batch_size):
        state_size = [batch_size, self.num_features] + [self.shape[0]]+[self.shape[1]]
...
class CLSTM(nn.Module):
    ...
    def __init__(self, shape, input_chans, filter_size, num_features, num_layers):
        ...
        cell_list = []
        cell_list.append(
            CLSTM_cell(self.shape, self.input_chans, self.filter_size, self.num_features[0]))  # the first one has a different number of input channels

        for idcell in range(1, self.num_layers):
            cell_list.append(CLSTM_cell(self.shape, self.num_features[idcell-1], self.filter_size, self.num_features[idcell]))
